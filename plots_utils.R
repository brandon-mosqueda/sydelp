...
baseline_metric <- function(metric, dataset) {
  Baseline %>%
...
plot_xy <- function(Data, x, y, dataset) {
  ...
  geom_hline(
    yintercept = baseline_metric(y, dataset),
    color = "red",
    linetype = 'dashed',
    size = 0.3
  ) +
...
mnist_plots <- function(Data, group) {
  ...
  save_plot(accuracy_plot, "plots/final/MNIST/accuracy.png")

  att_sr_plot <- plot_att_succ_rate(Data, dataset = "MNIST")
  save_plot(att_sr_plot, "plots/final/MNIST/attack_success_rate.png")
}
...
spam_plots <- function(Data, group) {
  ...
  save_plot(accuracy_plot, "plots/final/SMS Spam/accuracy.png")
  ...
  save_plot(f1_score_plot, "plots/final/SMS Spam/f1_score.png")
}
